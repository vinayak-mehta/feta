...
services = {
    "pypi": {"name": "PyPI", "url": "https://pypi.org/pypi/{project_name}/json"},
    "rtfd": {"name": "Read The Docs", "url": "https://{project_name}.readthedocs.io"},
}
...
def find_project(service_name, project_name):
    service = services[service_name]

    with Halo(text=service["name"], spinner="dots") as spinner:
        r = requests.get(service["url"].format(project_name=project_name))
        if r.status_code == 200:
            spinner.fail(f"{service['name']} (not available)")
        else:
            spinner.succeed(f"{service['name']} (available)")
